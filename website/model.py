...
def runAssign(assignments, timeslots, user):
    ...
    for sID in range(1, (num_slots) + 1):
        solver.Add(
            solver.Sum([x[aID, sID] for aID in range(1, (num_assignments) + 1)]) == 1
        )
        ...
        solver.Add(
            solver.Sum(
                [
                    ...
                    * (
                        (
                            timeslots[sID - 1]["endTime"]
                            - timeslots[sID - 1]["startTime"]
                        ).seconds
                        // 60
                    )
                    for sID in range(1, (num_slots) + 1)
...
def runTest(assignments, timeslots):
    ...
    for a in all_assignments:
        time = 0
        solver.Add(
            datetime.timedelta(minutes=sum(assign[(a, s)] for s in all_slots) * 25)
            >= assignments[a].get("amtOfTime")
        )
